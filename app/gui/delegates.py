...
app_logger = logging.getLogger("PixelHand.gui.delegates")
...
class ImageItemDelegate(QStyledItemDelegate, PaintUtilsMixin):
    ...
    def _draw_text_info(self, painter, option, item_data):
        ...
        if self.is_grid_mode:
            ...
            grid_meta_lines.append(f"{item_data.format_str} ({item_data.bit_depth}b)")
            if item_data.color_space:
                grid_meta_lines.append(item_data.color_space)
            grid_meta_lines.append(f"{item_data.texture_type} | Mips: {item_data.mipmap_count}")

            full_grid_meta = "\n".join(grid_meta_lines)

            meta_rect = QRect(x, y, w, 100)
            painter.drawText(
                meta_rect,
                Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignHCenter | Qt.TextFlag.TextWordWrap,
                full_grid_meta,
            ...
            painter.drawText(
                x,
                y,
                self.regular_font_metrics.elidedText(full_meta_line, Qt.TextElideMode.ElideRight, r.width()),
            )
            ...
            painter.drawText(
                x,
                y,
                self.regular_font_metrics.elidedText(str(path.parent), Qt.TextElideMode.ElideRight, r.width()),
            )
