...
/// Mipmap level selection strategy for DDS decoders.
#[derive(Debug, Clone, Copy)]
pub enum MipSelectionStrategy {
    /// Selects a matching mipmap level based on a bounding box target size.
    Auto(Option<u32>),
    /// Selects an explicit, raw mipmap level index.
    Specific(u32),
}

/// Encapsulates low-level hardware byte manipulations and unswizzling algorithms.
pub struct DdsPayloadProcessor;

impl DdsPayloadProcessor {
    /// Unswizzles block linear compressed textures (typically generated by CryEngine).
        ...
        dst.par_chunks_mut(row_bytes)
            ...
            .for_each(|(y_idx, row_chunk)| {
                ...
                for x in 0..block_width {
                    ...
                    // Perform safe bounds-checked slice writes
                    if let Some(src_sub) = src.get(src_offset..src_offset + block_bytes)
                        && let Some(dst_sub) =
    ...
    /// Performs Xbox 360 big-endian to little-endian swapping on block data.
...
/// Parses the basic DDS header attributes safely.
...
/// Consolidated internal decoder routine that handles standard header patching,
/// unswizzling, and byte-swapping depending on the selected strategy.
